...
create table students_subjects(
  ...
  -- заметьте оценка студента по предмету
                                mark int
);
...
-- каков мин., макс., средний балл учеников
...
-- Группировка
-- Напишем запрос, который выводит средний бал по предмету:

select s.name, avg(ss.mark)
from students_subjects as ss join subjects s
 ...
 */
/*
В данном случае признаком выступает имя предмета,
по нему производиться группировка, группы же
будут состоять из других столбцов.
В частности, среди этих групп окажется оценка
по предмету, к которой мы и применяем
агрегатную функцию.
 */
